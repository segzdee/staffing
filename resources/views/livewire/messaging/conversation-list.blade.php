    ...
    <div class="p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between mb-4">
            ...
            <button
                ...
                class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300"
            >
        ...
        <div class="relative">
            <input
                ...
                class="w-full px-4 py-2 pl-10 text-sm border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
        ...
        <div class="flex gap-2 mt-4 overflow-x-auto">
            <button
                ...
                class="px-3 py-1 text-sm rounded-full {{ $filter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' }}"
            >
                All
            </button>
            <button
                wire:click="setFilter('direct')"
                class="px-3 py-1 text-sm rounded-full {{ $filter === 'direct' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' }}"
            >
                Direct
            </button>
            <button
                wire:click="setFilter('shift')"
                class="px-3 py-1 text-sm rounded-full {{ $filter === 'shift' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' }}"
            >
                Shifts
            </button>
            <button
                wire:click="setFilter('support')"
                class="px-3 py-1 text-sm rounded-full {{ $filter === 'support' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' }}"
            >
                Support
            </button>
        </div>
    ...
    <div class="flex-1 overflow-y-auto">
        @forelse($this->conversations as $conversation)
            <div
                ...
                class="flex items-start gap-3 p-4 cursor-pointer border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors {{ $selectedConversationId === $conversation->id ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}"
            >
                {{-- Avatar --}}
