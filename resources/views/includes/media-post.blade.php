@if ($mediaImageVideoTotal == 1)
	...
	@if ($media->video != '' && $media->uploadedto_aws=="N")
	<video class="js-player w-100 @if (!request()->ajax())invisible @endif" controls @if ($media->video_poster) data-poster="{{ config('path.videos').$media->video_poster }}" @endif>
		<source src="{{ config('path.videos').$media->video }}" type="video/mp4" />
	</video>
	@endif

	@if ($media->video != '' && $media->uploadedto_aws=="Y")
	<video class="js-player w-100 @if (!request()->ajax())invisible @endif" controls @if ($media->video_poster) data-poster="{{ Helper::getFile(config('path.aws_videos').$media->video_poster) }}" @endif>
		<source src="{{Helper::getFile(config('path.aws_videos').$media->video )}}" type="video/mp4" />
	</video>
	@endif
...
@foreach ($mediaImageVideo as $media)
	...
	if ($media->type == 'video') {
		...
		if ($media->uploadedto_aws=="N"){
			$urlMedia =  config('path.videos').$media->video;
		    $videoPoster = $media->video_poster ? config('path.videos').$media->video_poster : false;
