...
<a href="{{ route('business.profile') }}" class="flex items-center space-x-3 px-3 py-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
    </svg>
    <span>Profile</span>
</a>
<a href="{{ route('business.ratings.breakdown') }}" class="flex items-center space-x-3 px-3 py-2 text-gray-900 bg-brand-50 rounded-lg font-medium">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="p-6">
    <div class="max-w-6xl mx-auto">
        {{-- Page Header --}}
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-900">Business Rating Breakdown</h1>
            <p class="text-gray-600 mt-1">See how workers have rated your business across different categories.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-brand-50 rounded-xl border border-brand-200 p-6">
                    ...
                    <p class="text-sm text-brand-700 mb-3">Your weighted score is calculated based on worker feedback in each category:</p>
                ...
                <div class="bg-green-50 rounded-xl border border-green-200 p-6">
                    <h3 class="text-sm font-semibold text-green-800 mb-3">Tips to Improve Ratings</h3>
                    <ul class="text-sm text-green-700 space-y-2">
                        <li class="flex items-start gap-2">
                            <svg class="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                            <span>Provide clear shift instructions upfront</span>
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                            <span>Start and end shifts on schedule</span>
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                            <span>Respond promptly to worker questions</span>
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                            <span>Release payments promptly after shift completion</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="mt-6 bg-white rounded-xl border border-gray-200 p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">Recent Feedback from Workers</h2>
                            ...
                            <div class="flex-shrink-0">
                                ...
                                @else
                                    <div class="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 font-semibold">
                                        {{ strtoupper(substr($rating->rater?->name ?? 'W', 0, 1)) }}
                                    </div>
                                @endif
                            </div>
                            <div class="flex-1 min-w-0">
                                ...
                                @if ($rating->response_text)
                                    <div class="mt-3 p-3 bg-white rounded border border-gray-200">
                                        <p class="text-xs font-medium text-gray-500 mb-1">Your Response:</p>
                                        <p class="text-sm text-gray-700">{{ $rating->response_text }}</p>
                                    </div>
                                @else
                                    <div class="mt-3">
                                        <button
                                            type="button"
                                            onclick="document.getElementById('response-form-{{ $rating->id }}').classList.toggle('hidden')"
                                            class="text-sm text-brand-600 hover:text-brand-700"
                                        >
                                            Respond to this review
                                        </button>
                                        <form
                                            id="response-form-{{ $rating->id }}"
                                            action="{{ route('ratings.respond', $rating->id) }}"
                                            method="POST"
                                            class="hidden mt-2"
                                        >
                                            @csrf
                                            <textarea
                                                ...
                                                rows="2"
                                                maxlength="500"
                                                class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-transparent"
                                                placeholder="Write your response..."
                                                required
                                            ></textarea>
                                            <div class="flex gap-2 mt-2">
                                                <button type="submit" class="px-3 py-1 text-sm bg-brand-600 text-white rounded hover:bg-brand-700">
                                                    Submit Response
                                                </button>
                                                <button
                                                    type="button"
                                                    onclick="document.getElementById('response-form-{{ $rating->id }}').classList.add('hidden')"
            ...
            @else
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-sm mt-1">Post shifts to start receiving feedback from workers.</p>
                </div>
            @endif
        </div>
    </div>
