...
<div class="p-6 space-y-6">
    <!-- Header -->
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-1">My Badges & Achievements</h2>
        <p class="text-gray-500">Track your progress and earned achievements</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        @foreach($badgeDefinitions as $badgeType => $badge)
            ...
            <div class="bg-white rounded-xl border-2 {{ $earned ? 'border-yellow-400 shadow-lg' : 'border-gray-200' }} p-6 transition-all hover:shadow-xl">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center space-x-4">
                        <div class="w-16 h-16 {{ $earned ? 'bg-yellow-100' : 'bg-gray-100' }} rounded-full flex items-center justify-center">
                            @if($earned)
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <h3 class="text-lg font-bold text-gray-900 mb-4">Recently Earned</h3>
        <div class="space-y-3">
            @foreach($recentBadges as $badge)
                <div class="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div class="flex-1">
                        <h4 class="font-semibold text-gray-900">{{ $badgeDefinitions[$badge->badge_type]['name'] ?? $badge->badge_type }}</h4>
                        <p class="text-sm text-gray-500">Earned {{ $badge->earned_at->diffForHumans() }}</p>
                    </div>
                </div>
            @endforeach
        </div>
