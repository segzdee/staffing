...
<nav class="space-y-1">
    @foreach($navigation as $key => $item)
        @if(is_string($key) && is_array($item))
            ...
            <div class="px-3 mb-2 mt-4 first:mt-0">
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $key }}</p>
            </div>
            @foreach($item as $subItem)
                @if(Route::has($subItem['route']))
                    ...
                    @endphp
                    <a href="{{ route($subItem['route']) }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors mb-1
                                            {{ $isActive
        ...
        @else
            ...
            @if(Route::has($item['route']))
                ...
                @endphp
                <a href="{{ route($item['route']) }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors mb-1
                                    {{ $isActive
                            ? 'bg-gray-900 text-white'
                            : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if(Route::has('messages.index'))
        <a href="{{ route('messages.index') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
    ...
    @if(Route::has('dashboard.settings'))
        <a href="{{ route('dashboard.settings') }}" class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
