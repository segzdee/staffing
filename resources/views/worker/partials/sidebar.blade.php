...
<a href="{{ route('api.market.index') }}"
    ...
    <span class="truncate">Live Market</span>
</a>
...
<a href="{{ route('worker.assignments.index') }}"
    @click="if(window.innerWidth < 1024) closeSidebar()"
    class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors {{ request()->routeIs('worker.assignments*') ? 'active' : '' }}">
    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
    </svg>
    <span class="truncate">My Assignments</span>
</a>

<a href="{{ route('worker.applications') }}"
    @click="if(window.innerWidth < 1024) closeSidebar()"
    class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors {{ request()->routeIs('worker.applications*') ? 'active' : '' }}">
    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<a href="{{ route('worker.calendar') }}"
    @click="if(window.innerWidth < 1024) closeSidebar()"
    class="sidebar-link flex items-center space-x-3 px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors {{ request()->routeIs('worker.calendar*') ? 'active' : '' }}">
    <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<a href="{{ route('shifts.index') }}?recommended=1"
    ...
    <span class="truncate">Recommended</span>
</a>
