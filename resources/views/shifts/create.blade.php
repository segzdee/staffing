                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date"
                               ...
                               @change="validateField('shift_date'); checkHoliday()"
                               required
                        ...
                        <!-- GLO-007: Holiday Indicator -->
                        <div x-show="holidayInfo && holidayInfo.is_holiday"
                             x-transition
                            ...
                            <div class="flex items-start gap-2">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-amber-800" x-text="holidayInfo.holiday ? holidayInfo.holiday.name : 'Public Holiday'"></p>
                                    <p class="text-xs text-amber-600 mt-1">
                                        <span>Surge pricing applies: </span>
                                        <span class="font-semibold" x-text="'+' + ((holidayInfo.surge_multiplier - 1) * 100).toFixed(0) + '%'"></span>
                                        <span class="ml-2" x-show="holidayInfo.holiday && holidayInfo.holiday.type_label" x-text="'(' + holidayInfo.holiday.type_label + ')'"></span>
                                    </p>
                                </div>
                            </div>
                        </div>
                        <p x-show="checkingHoliday" class="text-xs text-gray-500 mt-1 flex items-center gap-1">
...
@endphp
        ...
        // GLO-007: Holiday indicator state
        holidayInfo: null,
        checkingHoliday: false,
        countryCode: '{{ auth()->user()->businessProfile?->country ?? "US" }}',
        ...
        // GLO-007: Check if the selected date is a public holiday
        async checkHoliday() {
            const date = this.form.shift_date;
            if (!date) {
                this.holidayInfo = null;
                return;
            }

            this.checkingHoliday = true;
            this.holidayInfo = null;

            try {
                const region = this.form.location_state || null;
