...
$(document).on('click','.checkSubscription-btn',function(){
    var model_id = $(this).attr('user_id');
     var mediaId=($(this).attr("data-mediaid"));
     var amount=$(this).attr("data-pricegross");

    $.ajax({
        type:"GET",
        url:"/checkSubscription/"+model_id,

        success:function(response)
        {
            if(response == 'subscribed')
                {
                    $('#payPerViewForm').modal('toggle');
                    $("#formSendPPV .mediaIdInput").val(mediaId);
                    $("#formSendPPV .priceInput").val(amount);
                }
            else
            {
                swal({
                   title: 'Warning',
                    type: "warning",
                    text: 'You are not Subscribed to Unlock this Content.',
                    confirmButtonText: 'OK',
                    }
                );
            }
