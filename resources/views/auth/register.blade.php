...
@section('form')
    <div class="space-y-6 px-4 sm:px-0"
        ...
        <form action="{{ route('register') }}" method="POST" class="space-y-5">
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-3 gap-2 sm:gap-3">
                    <label
                        class="relative flex items-center justify-center p-4 sm:p-3 border rounded-lg cursor-pointer transition-all hover:bg-accent touch-manipulation"
            ...
            <div class="flex items-start gap-3">
                <input
                    ...
                    id="agree_terms"
                    name="agree_terms"
                    class="h-5 w-5 sm:h-4 sm:w-4 mt-0.5 rounded border-input text-primary focus:ring-primary focus:ring-offset-0 flex-shrink-0"
                    required
        ...
        @php
            ...
            $gridCols = $enabledCount === 1 ? 'grid-cols-1' : ($enabledCount === 2 ? 'grid-cols-2' : 'grid-cols-1 sm:grid-cols-3');
        @endphp
        ...
        <div class="grid {{ $gridCols }} gap-3">
            @if($googleEnabled)
            <x-ui.button variant="outline" class="w-full h-12 sm:h-10 text-base sm:text-sm" as="a" ::href="socialUrls.google + '?action=register&type=' + userType">
                <svg class="h-5 w-5 sm:h-4 sm:w-4 mr-2" viewBox="0 0 24 24">
            ...
            @endif
            @if($appleEnabled)
            <x-ui.button variant="outline" class="w-full h-12 sm:h-10 text-base sm:text-sm" as="a" ::href="socialUrls.apple + '?action=register&type=' + userType">
                <svg class="h-5 w-5 sm:h-4 sm:w-4 mr-2" fill="currentColor" viewBox="0 0 24 24">
            ...
            @endif
            @if($facebookEnabled)
            <x-ui.button variant="outline" class="w-full h-12 sm:h-10 text-base sm:text-sm" as="a" ::href="socialUrls.facebook + '?action=register&type=' + userType">
                <svg class="h-5 w-5 sm:h-4 sm:w-4 mr-2" fill="currentColor" viewBox="0 0 24 24">
        ...
        {{-- Switch to Login --}}
        <p class="text-center text-sm text-muted-foreground pb-safe">
