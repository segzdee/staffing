...
@section('title', 'Create Data Region')
@section('page-title', 'Create Data Region')
@section('page-subtitle', 'Add a new geographic data storage region')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <form method="POST" action="{{ route('admin.data-residency.store-region') }}">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text"
                                   ...
                                   class="w-full h-10 px-4 text-sm text-gray-900 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent @error('code') border-red-500 @enderror"
                                   required>
                            @error('code')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500">Lowercase, max 10 characters</p>
                        </div>
                        <div>
                        ...
                        @foreach($complianceFrameworks as $code => $label)
                            <label class="flex items-center p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100">
                                <input type="checkbox"
                                       ...
                                       {{ in_array($code, old('compliance_frameworks', [])) ? 'checked' : '' }}>
